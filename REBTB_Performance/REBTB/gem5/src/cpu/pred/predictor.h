...
#define LOOPTAG 10		   // tag width in the loop predictor
		...
		predictorsize()
		{
			...
			int inter = (1 << LOGL) * (2 * WIDTHNBITERLOOP + LOOPTAG + 4 + 4 + 1);
		...
		public:
			...
			// printf("Pred %#llx: %d, %d, %d\n", PC, HitBank, AltBank, tage_pred);
			// printf("Pred BI:%x, G[1]:%i, G[2]:%i \n", BI, GI[1], GI[2]);
...
// the replacement is optimized with a single u bit: 0.2 %
				...
				int g = 0;
				for (int i = 0; i < 16; i++) {
					g |= (ghist[(ptghist + i) & (HISTBUFFERLENGTH - 1)] << i);
				}
				// printf("Committing branch: %#llx: GHR:%#x\n", PC, g);
