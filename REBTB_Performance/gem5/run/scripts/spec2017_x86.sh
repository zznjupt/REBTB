...
OUTPUT_DIR=/root/gem5/run/output_x86 # Directory to place run output. Make sure this exists!
######################### BENCHMARK CODENAMES ####################
...
if [[ "${BENCHMARK:0-1:1}" == "r" ]]; then
    RUN_DIR=$SPEC_DIR/benchspec/CPU/$BENCHMARK_CODE/run/run_base_refrate\_gem5-x86.0000     # Run directory for the selected SPEC benchmark
fi
if [[ "${BENCHMARK:0-1:1}" == "s" ]]; then
    RUN_DIR=$SPEC_DIR/benchspec/CPU/$BENCHMARK_CODE/run/run_base_refspeed\_gem5-x86.0000     # Run directory for the selected SPEC benchmark
fi
...
$GEM5_DIR/configs/se_spec2017_x86.py \
$FLAGS \
--benchmark=$BENCHMARK \
