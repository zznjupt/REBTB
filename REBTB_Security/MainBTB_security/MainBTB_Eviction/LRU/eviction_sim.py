...
def simulate_eviction(i=10, t=9, w=16, x_start=0, x_end=30000, x_step=2000,
    ...
    cdf_theory = [np.mean(T_samples <= n) for n in N_theory]

    # --- Simulated experiments with empty initial target set ---
    trials = []
    for _ in range(num_experiments):
        target = np.random.randint(0, space_size)
        bucket = deque([], maxlen=w) 
        total_trials = 0
        new_labels = 0
        ...
        while new_labels < w:
            ...
            if guess == target:
                ...
                existing_same_tag = [off for t, off in bucket if t == guess_tag]
                
                if not existing_same_tag or all(off < guess_offset for off in existing_same_tag):
                    bucket.append((guess_tag, guess_offset))
                    new_labels += 1
