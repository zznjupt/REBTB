...
from aliasing_sim import simulate_eviction  # Your parameterized script
...
def plot_eviction_case(param_name, param_values, fixed_params, x_start, x_end, x_step, filename, add_inset=False):
    """Plot eviction CDF for a range of parameter values, reading CSV if available."""
    ...
    for idx, val in enumerate(param_values):
        ...
        all_data.append((color, marker, N_theory, cdf_theory, xx_ticks, cdf_sim_ticks))

        legend_str = f"{params['i']}-bit index, {params['t']}-bit tag, {params['w']}-way, 5-bit offset"
    ...
    if add_inset:
        ...
        for color, marker, N_theory, cdf_theory, xx_ticks, cdf_sim_ticks in all_data:
...
# --- Case 1: i=10, w=12, t=[6,7,8,9] with inset ---
plot_eviction_case(
    param_name='t',
    param_values=[6,7,8,9],
    fixed_params={'i':10,'w':12},
    x_start=0, x_end=448000,
    x_step=32000,
    filename='case1_i10_w12_t6-9.pdf',
)
